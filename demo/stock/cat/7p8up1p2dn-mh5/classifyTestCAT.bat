...
classifyFiles -train=data/cat-7p8up1p2dn-mh5-close.train.csv -test=data/cat-7p8up1p2dn-mh5-close.test.csv -testOut=tmpout/cat-7p8up1p2dn-mh5-close.out.csv -minBuck=290  -maxBuck=300 -detToStdOut=false -doPreAnalyze=false -writeFullcsv=true  -IgnoreColumns=symbol,datetime,sam20,sl90,sl6,sl60,ram30,ram20,sl12,sbm10,rbm10,sam10,sam90,sam180,sam360,sbm90,sbm180,sbm360

:: -writeFullcsv=true causes larger output that includes the symbol and bar datetime in teh output file  tmpout/cat-7p8up1p2dn-mh5-close.out.csv which is dereived from the filename passed into -testout.
